...
public class Visualizer extends JPanel{  
    ...
    public Visualizer (ArrayList<Integer>[] xCoords, ArrayList<Integer>[] yCoords,String xAxisLabel,String yAxisLabel) {
    ...
    protected void paintComponent(Graphics grf){  
      ...
      //--------- now let's handle the y ticks -------------
        double[] goodTickLabels = new double[] {10000,1000,500,200,100,50,20,10,5, 2, 1, 0.5, 0.2, 0.1, 0.05,0.01,0.005, 0.001};
        ...
        for (int series=0; series<yCoords.length; series++) {
	        ...
	        if (xCoords==null || xCoords[series]==null){
		        for(int i=0; i<yCoords[series].size(); i++){  
		        	x1 = marg+i*scaleX;  
		            y1 = height-marg-scaleY*yCoords[series].get(i);  
		            graph.fill(new Ellipse2D.Double(x1-2, y1-2, 4, 4));  
		        }  
	        }
	        else {
	        	 for(int i=0; i<yCoords[series].size(); i++){  
	 	        	x1 = marg+xCoords[series].get(i)*scaleX;  
	 	            y1 = height-marg-scaleY*yCoords[series].get(i);  
	 	            graph.fill(new Ellipse2D.Double(x1-2, y1-2, 4, 4));  
	 	        }  
	        }
    ...
    private double getMax(ArrayList<Integer>[] xCoords2){  
        double max = -Double.MAX_VALUE;  
        for(int i=0; i<xCoords2.length; i++){  
	        for(int j=0; j<xCoords2[i].size(); j++){  
	            if(xCoords2[i].get(j)>max)  
	                max = xCoords2[i].get(j);  
    ...
    public static void visualize(ArrayList<Integer>[] xCoords, ArrayList<Integer>[] yCoords, String xLabel, String yLabel) {
    	 //create an instance of JFrame class  
        JFrame frame = new JFrame();  
        // set size, layout and location for frame.  
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);  
        JPanel mainPanel = new Visualizer(xCoords, yCoords,xLabel,yLabel);  
        frame.add(mainPanel); 
